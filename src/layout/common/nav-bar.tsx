  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-[200] flex h-20 flex-row items-center justify-between bg-white py-3 px-2 shadow-md sm:px-3 md:px-4 md:py-2 lg:py-3 lg:px-6 xl:px-9 2xl:px-16">
        <Link
          href="/"
          className="grid h-full w-max grid-cols-[max-content] grid-rows-[1fr_min-content] items-start justify-start gap-1 no-underline"
        >
          <svg
            viewBox="0 0 157 33"
            fill="none"
        ...
        <div className="grid h-full grid-cols-[repeat(2,_max-content)] grid-rows-[1fr] items-center justify-end gap-x-2 sm:gap-x-4 md:gap-x-8 lg:gap-x-9 xl:gap-x-12">
            ...
            <div className="hidden flex-row items-center justify-end gap-x-12 xl:flex">
              {navLinks.map((link) => (
                <Link
                  key={link.route}
                  href={link.route}
                  className="relative font-sans text-xs font-bold uppercase leading-snug tracking-[0.2em] text-black-800 no-underline transition-[color] duration-200 ease-in-out hover:text-primary"
                >
                  {link.label}
                </Link>
              ))}
          ...
          <button
            className="focus-visible::outline flex aspect-square h-full items-center justify-center p-2 outline-water-200"
            onClick={toggleSideMenu}
          >
            <svg
              viewBox="0 0 33 20"
        ...
        {sideMenuOpen && (
          ...
          >
            <div className="grid h-16 grid-cols-[minmax(min-content,_1fr)_min-content] ">
              ...
              <button
                onClick={toggleSideMenu}
                className="flex h-14 w-14 items-center justify-center border-none outline-water-300 focus-visible:outline "
              >
            ...
            <div className="flex flex-col items-stretch justify-start gap-y-1 md:gap-y-2 lg:gap-y-3">
              {sideBarNavLinks.map((link) =>
                link.external ? (
                  <a
                    className="rounded-lg bg-transparent px-6 py-4 text-right text-xl font-bold tracking-wider text-black-800 outline-water-300 transition-colors duration-200 ease-in-out hover:bg-primary-100 hover:text-primary focus-visible:outline"
                    key={link.route}
                    href={link.route}
                    target="_blank"
                    rel="noreferrer"
                  ...
                  <Link
                    ...
                    key={link.route}
                    className={twMerge(
                      "rounded-lg px-6  py-4 text-right text-xl font-bold tracking-wider text-black-800 outline-water-300 hover:bg-primary-100 hover:text-primary focus-visible:outline-1",
